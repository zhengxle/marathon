#!/bin/bash
# This script runs the scale tests against a marathon instance that uses
# the internal mesos-simulator. This requires no cluster, but do require
# a valid devleopment environment.

# Get the root dir of the current marathon directory
MARATHON_DIR=$(git rev-parse --show-toplevel)
...
for TEST_CONFIG in ./config/test-*.yml; do
  ...
  eval dcos-perf-test-driver \
    ./config/using-local-simulator.yml \
    ./config/ci-specific-config.yml \
    $TEST_CONFIG \
    $REST_ARGS \
    -D "marathon_repo_dir=${MARATHON_DIR}" \
