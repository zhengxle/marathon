...
class QueueResource @Inject() (
  ...
  def index(@Context req: HttpServletRequest, @QueryParam("embed") embed: java.util.Set[String]): Response = authenticated(req) { implicit identity =>
    ...
    val infos = launchQueue.listWithStatistics.filter(t => t.runSpec.isDefined).filter(t => t.inProgress && isAuthorized(ViewRunSpec, t.runSpec.get))
  ...
  def resetDelay(
    ...
    val maybeApp = launchQueue.list.find(_.runSpec.map(_.id).contains(appId)).flatMap(_.runSpec)
