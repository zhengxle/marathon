...
/**
  ...
  * calls are not guaranteed to be executed in any particular order the same way one will not expect two independent futures
  * to have an order.
  *
  ...
  *
  * @param store underlying ZK persistence store
...
class CachedTemplateRepository(
  ...
  val cache: AsyncLoadingCache[String, Versioned] =
    Scaffeine()
      .recordStats()
  ...
  override def create(template: Template): Future[Versioned] = async {
    val versioned = await(super.create(template))
    cache.put(toCacheKey(versioned), Future.successful(versioned))
    versioned
  }

  override def read(pathId: PathId, version: Int): Future[Versioned] =
...
object CachedTemplateRepository {
  def toCacheKey(versioned: Versioned): String = toCacheKey(versioned.pathId, versioned.version)
