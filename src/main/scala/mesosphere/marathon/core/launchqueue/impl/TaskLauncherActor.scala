...
private class TaskLauncherActor(
  ...
  def instancesToLaunch = instanceMap.valuesIterator.count(_.isScheduled)
  ...
  private[this] def inFlight(op: InstanceOp): Boolean = inFlightInstanceOperations.exists(_.instanceId == op.instanceId)
