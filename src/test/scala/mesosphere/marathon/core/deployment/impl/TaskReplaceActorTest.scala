...
class TaskReplaceActorTest extends AkkaUnitTest with Eventually {
  "TaskReplaceActor" should {
    "Replace without health checks" in {
      ...
      f.scheduler.add(eq(newApp), eq(5))(any) returns Future.successful(Done)
    ...
    "New and already started tasks should not be killed" in {
      ...
      val promise = Promise[Unit]()
      val newApp = app.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(1)))
      f.scheduler.add(eq(newApp), eq(4))(any) returns Future.successful(Done)
    ...
    "Replace and scale down from more than new minCapacity" in {
      ...
      val promise = Promise[Unit]()
      val newApp = app.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(1)))
      f.scheduler.add(eq(newApp), eq(1))(any) returns Future.successful(Done)
      ...
      ref ! f.instanceChanged(newApp, Running)
      eventually { app: AppDefinition => verify(f.scheduler, times(2)).add(app) }
    ...
    "Replace with minimum running tasks" in {
      ...
      val promise = Promise[Unit]()
      val newApp = app.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(1)))
      f.scheduler.add(eq(newApp), eq(3))(any) returns Future.successful(Done)
      val ref = f.replaceActor(newApp, promise)
      watch(ref)

      // all new tasks are queued directly
      eventually { app: AppDefinition => verify(f.scheduler, times(3)).add(app) }
    ...
    "Replace with rolling upgrade without over-capacity" in {
      ...
      eventually {
        queueOrder.verify(f.scheduler).add(_: AppDefinition, 1)
      }
    ...
    "Replace with rolling upgrade with minimal over-capacity" in {
      ...
      queueOrder.verify(f.scheduler, never).add(_: AppDefinition, 1)
    ...
    "Replace with rolling upgrade with 2/3 over-capacity" in {
      ...
      val promise = Promise[Unit]()
      val newApp = app.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(1)))
      f.scheduler.add(eq(newApp), any)(any) returns Future.successful(Done)
      val ref = f.replaceActor(newApp, promise)
      watch(ref)
      ...
      val queueOrder = org.mockito.Mockito.inOrder(f.scheduler)
      eventually {
        queueOrder.verify(f.scheduler).add(_: AppDefinition, 2)
      }
    ...
    "Downscale with rolling upgrade with 1 over-capacity" in {
      ...
      eventually {
        verify(f.scheduler, times(2)).add(eq(newApp), eq(1))(any)
      }
      ...
      eventually {
        verify(f.scheduler, times(3)).add(eq(newApp), eq(1))(any)
      }
    ...
    "Wait until the tasks are killed" in {
      ...
      val promise = Promise[Unit]()
      val newApp = app.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(1)))
      f.scheduler.add(newApp, 5) returns Future.successful(Done)
    ...
    "Tasks to replace need to wait for health and readiness checks" in {
      ...
      val promise = Promise[Unit]()
      val newApp = app.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(1)))
      f.scheduler.add(newApp, 1) returns Future.successful(Done)
      val ref = f.replaceActor(newApp, promise)
      watch(ref)
  ...
  class Fixture {
    ...
    def replaceActor(app: AppDefinition, promise: Promise[Unit]): ActorRef = system.actorOf(
      TaskReplaceActor.props(deploymentsManager, deploymentStatus, killService, scheduler, system.eventStream,
