...
/**
  * Run simple framework that:
  *  - listens to simple HTTP requests to launch tasks
  *  - a task is launched when a fitting offer is received
  *  - should a task fail, it is not restarted
  *  - an existing/running tasks can be killed and removed from the state
  *
  * This is a perfect show case for "one-off" tasks which don't have be kept alive (like Marathon does).
  * A failing/finished task is "ignored" - it is up to the user to launch a replacement.
  *
  */
object SimpleFramework extends App with StrictLoggingFlow {
